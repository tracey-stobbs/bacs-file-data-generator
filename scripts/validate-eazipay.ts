...
function validateRows(rows: string[][]) {
  ...
  rows.forEach((cols, i) => {
    if (cols.length !== expectedCols) errors.push(`Row ${i} has ${cols.length} cols (expected ${expectedCols})`);
    const txn = cols[0];
    if (!EaziPayValidator.isTransactionCode(txn)) errors.push(`Row ${i} invalid transaction code: "${txn}"`);
    ...
    if (!allowsSun && (sunCol && sunCol.trim() !== '')) {
      errors.push(`Row ${i} has SUN value but transaction ${txn} does not allow SUN: "${sunCol}"`);
    }
  });
  return errors;
}
...
(async function main(){
  ...
  const files = fs.existsSync(generatedPath) ? fs.readdirSync(generatedPath) : [];
