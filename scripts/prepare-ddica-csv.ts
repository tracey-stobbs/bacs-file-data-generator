...
function usage(): void {
  console.error(
    "Usage: prepare-ddica-csv.ts --srcCsv=path --outCsv=path [--metadata=path]",
  );
  process.exit(2);
}

function parseArgs(): { srcCsv: string; outCsv: string; metadata?: string } {
  const opts: { srcCsv?: string; outCsv?: string; metadata?: string } = {};
  ...
  // usage() exits if missing, so opts.srcCsv and opts.outCsv are present here
  const src = opts.srcCsv as string;
  const out = opts.outCsv as string;
  return { srcCsv: src, outCsv: out, metadata: opts.metadata };
}

function sanitizeAccountName(name: string): string {
  if (!name) return "";
  return name
    // remove characters outside printable ASCII range (0x20..0x7E)
    .replace(/[^\x20-\x7E]/g, "")
    .replace(/[",]+/g, "")
    .replace(/\s+/g, " ")
    .trim();
...
(function main(): void {
  ...
  const normalized = debitRows.map((r): string[] => {
    const copy = r.slice();
    ...
    copy[7] = String(
      Number(String(copy[7] || "0").replace(/[^0-9.-]+/g, "")) || 0,
    );
    return copy;
  });

  // Group by destination account (sort+acct) to find duplicates
  const groupKey = (r: string[]): string =>
    `${(r[3] || "").trim()}|${(r[4] || "").trim()}`;
  ...
  if (!hasMultiple) {
    ...
    const isoBase =
      base[8] && /^\d{4}-\d{2}-\d{2}$/.test(base[8])
        ? base[8]
        : normalizeDateToIso(base[8] || "");
  ...
  const mapped = outRows.map((r, idx): string[] => {
  ...
  const outContent = [header.join(","), ...mapped.map((r: string[]) => r.join(","))].join(
    "\n",
  );
