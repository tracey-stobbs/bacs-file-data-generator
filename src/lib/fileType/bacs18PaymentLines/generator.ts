...
export interface Bacs18GenerateRequest
  extends Omit<Bacs18PreviewRequest, "fileType"> {
  fileType: "Bacs18PaymentLines";
}
...
function julian(dt: DateTime): string {
...
export function generateValidBacs18Row(originating: {
  ...
  const amountPence =
    String(faker.number.int({ min: 1, max: 9_999_999 })) + "00";
  const tx = faker.helpers.arrayElement(
    Bacs18Validator.allowedTransactionCodes,
  );
  return {
    ...
    fixedZero: "0",
    transactionCode: tx,
    originatingSortCode: originating.sortCode,
    originatingAccountNumber: originating.accountNumber,
    realtimeInformationChecksum: faker.helpers.arrayElement([
      "0000",
      "ABCD",
      "EFGH",
    ]),
    amountPence: padLeft(amountPence, 11, "0").slice(-11),
    originatingAccountName: sanitize(originating.accountName)
      .padEnd(18, " ")
      .slice(0, 18),
    paymentReference: sanitize(
      faker.string.alphanumeric({ length: { min: 7, max: 17 } }),
    )
      .padEnd(18, " ")
      .slice(0, 18),
    destinationAccountName: sanitize(faker.company.name())
      .padEnd(18, " ")
      .slice(0, 18),
