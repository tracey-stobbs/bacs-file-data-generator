...
async function main(): Promise<void> {
  ...
  type ParsedRow = { fields: string[] };
  type Parsed = { rows: ParsedRow[] };
  const parsed = eaziPayAdapter.parse(content) as Parsed;
  console.log("Total rows:", parsed.rows.length);
  const bad = parsed.rows.filter((r: ParsedRow) => r.fields.length !== 14);
  console.log("Rows with non-14 columns:", bad.length);
  if (parsed.rows.length > 0) {
    const sample = parsed.rows.slice(0, 3).map((r: ParsedRow) => ({
      cols: r.fields.length,
      fields: r.fields.slice(0, 6),
    }));
    console.log("Sample rows (first 3):", JSON.stringify(sample, null, 2));
  }
  // rudimentary field checks
  const errors: string[] = [];
  parsed.rows.forEach((r: ParsedRow, idx: number) => {
    const f = r.fields;
    // transaction code
    if (typeof f[0] !== "string" || f[0].length === 0)
      errors.push(`row ${idx + 1}: missing txn code`);
    // sort codes numeric 6
    if (!/^\d{6}$/.test(f[1]))
      errors.push(`row ${idx + 1}: originating sort code invalid: "${f[1]}"`);
    if (!/^\d{8}$/.test(f[2]))
      errors.push(
        `row ${idx + 1}: originating account number invalid: "${f[2]}"`,
      );
    // amount numeric
    if (!/^-?\d+$/.test(f[7]))
      errors.push(`row ${idx + 1}: amount invalid: "${f[7]}"`);
  });
  if (errors.length === 0) {
